...
class Mx_cp_notice_ext {
	...
	public function __construct( $settings=FALSE ) {
		...
		$this->settings = array_merge( $this->default, (array)$this->settings );
	}
	...
	function cp_js_end( $data ) {
		...
		$out .=
			'$(function () {
	            	$.ee_notice = function (a, k, issue) {
	            		issue = typeof issue !== "undefined" ?  issue : false;
	            		Messenger.options = {
							...
							Messenger().post({
							  ...
							  actions: {
								    fixit: {
								      ...
								      phrase: "",
								      action: function() {
								      	$("#menu_" + issue.tab + ">a").trigger( "mousedown" );
								      	this.hide();
								      	$("html, body").animate({ scrollTop: issue.field.offset().top }, 1000);
								      	var $el = issue.field;
								      	//find(".publish_field");
								      	blink($el);
								      }
					...
					if(block_errors.length > 0)
					{
						for(e=0; e<block_errors.length; e++)
						{
							...
							$($("#" + block_errors[e]).find("div.notice")
							        .get()
							        .reverse())
							        .each(function() {
							       		var field_holder_id = $(this).parents(".publish_field:first");
										$.ee_notice(capitalizeWords(em) + " '.lang( 'tab' ).': " + $(this).html(),{type: "error",open:true}, {tab: block_errors[e], field: field_holder_id });
							});
						}
					}
            });
	...
	function activate_extension() {
		...
		$this->EE->mx_core->_saveSettingsToSession($this->default);
	}
